...
<title>Order Submission — ByTheBel</title>
<link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:ital,wght@0,300;0,400;0,500;1,300&display=swap" rel="stylesheet">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
<link rel="stylesheet" href="shop-styles.css">
<style>
.doc-wrap { max-width: 680px; margin: 0 auto; padding: 2.5rem 1.5rem 4rem; }
.doc-back {
  display: inline-flex; align-items: center; gap: 0.4rem;
  font-size: 0.8rem; color: var(--text-muted); background: none; border: none;
  font-family: 'DM Sans', sans-serif; cursor: pointer; transition: color 0.2s;
  margin-bottom: 1.6rem;
}
.doc-back:hover { color: var(--text); }
.doc-back svg { width: 15px; height: 15px; stroke: currentColor; fill: none; stroke-width: 2; }

.doc-card {
  background: var(--bg-card); border: 1px solid var(--border);
  border-radius: 14px; padding: 2rem; margin-bottom: 1.2rem;
}
.doc-heading { font-family: 'Bebas Neue', sans-serif; font-size: 1.6rem; letter-spacing: 0.08em; margin-bottom: 0.25rem; }
.doc-sub { font-size: 0.78rem; color: var(--text-muted); margin-bottom: 1.4rem; }

.rejected-banner {
  display: flex; align-items: flex-start; gap: 0.75rem;
  background: rgba(239,68,68,0.07); border: 1px solid rgba(239,68,68,0.25);
  border-radius: 10px; padding: 0.9rem 1rem; margin-bottom: 1.4rem; font-size: 0.84rem;
}
.rejected-banner svg { width: 18px; height: 18px; stroke: #ef4444; fill: none; stroke-width: 2; flex-shrink: 0; margin-top: 0.05rem; }
.rejected-banner strong { display: block; margin-bottom: 0.2rem; color: #ef4444; }
...
.info-grid {
  display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem 1.5rem; margin-bottom: 1.4rem;
}
@media (max-width: 480px) { .info-grid { grid-template-columns: 1fr; } }
.info-label { font-size: 0.65rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--text-muted); margin-bottom: 0.15rem; }
.info-value { font-size: 0.84rem; font-weight: 500; }

.doc-divider { border: none; border-top: 1px solid var(--border); margin: 1.2rem 0; }
...
.item-img { width: 48px; height: 56px; border-radius: 7px; object-fit: cover; background: var(--bg-section); flex-shrink: 0; }
.item-img-ph { width: 48px; height: 56px; border-radius: 7px; background: var(--bg-section); flex-shrink: 0; }
.item-name { font-size: 0.84rem; font-weight: 600; }
.item-meta { font-size: 0.74rem; color: var(--text-muted); margin-top: 0.15rem; }
.item-price { font-size: 0.86rem; font-weight: 600; margin-left: auto; flex-shrink: 0; }

.total-row { display: flex; justify-content: space-between; font-size: 0.83rem; padding: 0.4rem 0; }
.total-row.grand { font-weight: 700; font-size: 0.95rem; border-top: 1px solid var(--border); padding-top: 0.65rem; margin-top: 0.25rem; }
.total-label { color: var(--text-muted); }
.total-label.grand { color: var(--text); }

.proof-img-wrap { margin-top: 0.75rem; border-radius: 10px; overflow: hidden; border: 1px solid var(--border); cursor: zoom-in; }
.proof-img-wrap img { display: block; width: 100%; max-height: 300px; object-fit: contain; background: var(--bg-section); }
...
.btn-doc-back {
  ...
  font-size: 0.86rem; color: var(--text-muted); cursor: pointer; transition: all 0.2s;
}
.btn-doc-back:hover { border-color: var(--text); color: var(--text); }

.state-msg { text-align: center; padding: 5rem 2rem; color: var(--text-muted); font-size: 0.9rem; }

/* lightbox */
.lbx { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.9); z-index: 1200; align-items: center; justify-content: center; cursor: zoom-out; }
.lbx.open { display: flex; }
.lbx img { max-width: 92vw; max-height: 92vh; object-fit: contain; border-radius: 8px; }
</style>
</head>
<body>
...
async function loadSubmission(id) {
  try {
    ...
    document.getElementById('state-msg').innerHTML =
      `<div style="font-size:1.5rem;margin-bottom:1rem;opacity:0.4;">📋</div>
