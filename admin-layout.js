...
function _injectTopnav() {
  ...
  nav.innerHTML = `
    <div style="display:flex;align-items:center;gap:0.8rem;">
      ...
      <div class="topnav-logo">BuyTheBella</div>
    </div>
...
function _updateSidebarBadges(data) {
  ...
  if (payBadge) {
    payBadge.textContent =
      data.pending_payments > 9 ? "9+" : data.pending_payments;
    payBadge.style.display = data.pending_payments > 0 ? "" : "none";
  }
  if (revBadge) {
    revBadge.textContent =
      data.pending_reviews > 9 ? "9+" : data.pending_reviews;
...
function _injectSidebar(activeNavId) {
  ...
  aside.innerHTML = `
    <div class="sidebar-brand">BuyTheBella Admin</div>
