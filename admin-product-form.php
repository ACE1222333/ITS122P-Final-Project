...
<title>Product Form — Carousell Admin</title>
      ...
      <div class="form-card">
        ...
        <div class="form-row single">
          <div class="form-group">
            ...
            <div class="multi-drop-zone" id="f-drop"
                 onclick="document.getElementById('f-files').click()"
                 ondragover="handleMultiDragOver(event,'f-drop')"
                 ondragleave="handleMultiDragLeave(event,'f-drop')"
                 ondrop="handleFormDrop(event)">
              <input type="file" id="f-files" accept="image/*" multiple
                     onchange="handleFormFileSelect(event)">
      ...
      <div class="form-card" style="padding:1.2rem;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.75rem;">
          ...
          <button type="button" onclick="openAddCategoryModal()"
            style="background:none;border:none;color:var(--accent);font-family:'DM Sans',sans-serif;font-size:0.75rem;font-weight:600;cursor:pointer;letter-spacing:0.04em;padding:0;display:flex;align-items:center;gap:0.25rem;">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            New
          </button>
        </div>
        <div id="f-category-list" style="
          border:1px solid var(--border); border-radius:8px;
          max-height:320px; overflow-y:auto; padding:0.3rem 0;
          background:var(--bg); font-size:0.83rem;">
...
<div id="add-cat-overlay" style="display:none;position:fixed;inset:0;background:rgba(15,15,15,0.5);backdrop-filter:blur(3px);z-index:850;align-items:center;justify-content:center;">
  <div style="background:#fff;border-radius:12px;padding:2rem;width:380px;max-width:95vw;box-shadow:0 8px 30px rgba(0,0,0,0.18);">
    ...
    <div style="margin-bottom:0.85rem;">
      ...
      <input class="form-input" type="text" id="new-cat-name" placeholder="e.g. Vintage Tops" maxlength="100"
             onkeydown="if(event.key==='Enter') saveNewCategory()">
    </div>
    <div style="margin-bottom:1.4rem;">
      <label class="form-label" for="new-cat-desc">Description (optional)</label>
      <input class="form-input" type="text" id="new-cat-desc" placeholder="Short description"
             onkeydown="if(event.key==='Enter') saveNewCategory()">
    </div>
...
function getCheckedCategoryIds() {
  return Array.from(document.querySelectorAll('#f-category-list input[type=checkbox]:checked'))
...
async function initForm() {
  ...
  if (isEdit) {
    ...
    try {
      ...
      formCoverIndex = (typeof p.cover_index === 'number') ? p.cover_index
                     : (typeof p.coverIndex === 'number')  ? p.coverIndex : 0;
...
async function uploadFiles(files) {
  ...
  if (remaining <= 0) { showToast('Maximum 5 images reached.'); return; }
  const toUpload = files.filter(f => f.type.startsWith('image/')).slice(0, remaining);
  if (!toUpload.length) { showToast('Please select valid image files.'); return; }
  ...
  grid.innerHTML = formImages.map((src, i) => {
    ...
    return `
      <div class="img-thumb-wrap${isCover ? ' is-cover' : ''}">
        ...
        ${isCover
          ? `<span class="img-thumb-cover">Cover</span>`
...
function updatePreview() {
  ...
  if (formImages.length) {
    ...
    thumbsEl.innerHTML = formImages.map((src, i) =>
      `<img class="preview-thumb${i===formCoverIndex?' active':''}" src="${src}" alt="" onclick="setPreviewMain('${src}',this)">`
    ).join('');
  } else {
    mainImg.src = '';
    thumbsEl.innerHTML = '';
...
async function submitForm() {
  ...
  if (!condition)          { showToast('Please select a condition.'); return; }
