...
// Categories fetched dynamically from the API
...
function openConfirm(
  ...
  document.getElementById("confirm-ok").className = isDanger
    ? "btn-confirm-del"
    : "btn-save";
...
function conditionBadge(condition) {
  ...
  return condition
    ? `<span class="badge ${map[condition] || "badge-gray"}">${condition}</span>`
    : "—";
}
...
// Multi-image upload handled in admin-product-form via api/upload.php
/* Upload now happens immediately in admin-product-form.html via api/upload.php */
function handleMultiDragOver(e, dropId) {
  e.preventDefault();
  document.getElementById(dropId).classList.add("dragover");
}
function handleMultiDragLeave(e, dropId) {
