...
<title>Order Detail — ByTheBel Admin</title>
...
.detail-back {
  ...
  margin-bottom: 1.6rem; transition: color 0.2s;
}
.detail-back:hover { color: var(--text); }
.detail-back svg { width: 15px; height: 15px; stroke: currentColor; fill: none; stroke-width: 2; }
...
.detail-order-date { font-size: 0.78rem; color: var(--text-muted); margin-top: 0.3rem; }
...
.detail-card-title svg { width: 13px; height: 13px; stroke: var(--text-muted); fill: none; stroke-width: 2; flex-shrink: 0; }

.detail-row {
  display: flex; justify-content: space-between; align-items: flex-start;
  gap: 1rem; margin-bottom: 0.55rem; font-size: 0.83rem;
}
.detail-row:last-child { margin-bottom: 0; }
.detail-key { color: var(--text-muted); flex-shrink: 0; }
.detail-val { text-align: right; font-weight: 500; word-break: break-word; max-width: 62%; }
.detail-val.mono { font-family: monospace; font-size: 0.78rem; letter-spacing: 0.04em; }
...
.product-img { width: 56px; height: 66px; border-radius: 8px; object-fit: cover; background: var(--bg-section); flex-shrink: 0; }
.product-img-ph { width: 56px; height: 66px; border-radius: 8px; background: var(--bg-section); flex-shrink: 0; }
.product-name { font-size: 0.86rem; font-weight: 600; margin-bottom: 0.2rem; }
.product-meta { font-size: 0.75rem; color: var(--text-muted); }
.product-price { font-size: 0.86rem; font-weight: 600; margin-left: auto; flex-shrink: 0; }

.total-row {
  display: flex; justify-content: space-between; align-items: baseline;
  padding-top: 0.55rem; margin-top: 0.35rem; font-size: 0.83rem;
}
.total-row.grand {
  border-top: 1px solid var(--border); padding-top: 0.75rem; margin-top: 0.5rem; font-weight: 600;
}
.total-label { color: var(--text-muted); font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.05em; }
.total-label.grand { color: var(--text); font-size: 0.78rem; }
.total-amount-big { font-family: 'Bebas Neue', sans-serif; font-size: 1.6rem; letter-spacing: 0.06em; }

.proof-box {
  border-radius: 10px; overflow: hidden; border: 1px solid var(--border);
  cursor: pointer; position: relative; background: var(--bg-section); margin-top: 0.75rem;
}
.proof-box img { display: block; width: 100%; max-height: 240px; object-fit: cover; transition: opacity 0.2s; }
.proof-box:hover img { opacity: 0.85; }
.proof-overlay {
  position: absolute; inset: 0; display: flex; align-items: center; justify-content: center;
  background: rgba(0,0,0,0); transition: background 0.2s;
}
.proof-box:hover .proof-overlay { background: rgba(0,0,0,0.12); }
.proof-hint {
  background: rgba(0,0,0,0.6); color: #fff; font-size: 0.72rem; font-weight: 500;
  padding: 0.3rem 0.7rem; border-radius: 20px; opacity: 0; transition: opacity 0.2s;
}
.proof-box:hover .proof-hint { opacity: 1; }
.proof-none { font-size: 0.8rem; color: var(--text-muted); font-style: italic; padding: 0.4rem 0; }

/* rejection banner */
.rejection-banner {
  margin-top: 0.75rem; padding: 0.65rem 0.9rem;
  background: rgba(239,68,68,0.07); border: 1px solid rgba(239,68,68,0.25);
  border-radius: 8px; font-size: 0.8rem; color: var(--red,#dc2626); line-height: 1.5;
  display: none;
}
...
.action-btns { display: grid; grid-template-columns: 1fr 1fr; gap: 0.65rem; }
.btn-approve {
  background: var(--green); color: #fff; border: none; border-radius: 8px;
  padding: 0.8rem; font-family: 'DM Sans', sans-serif; font-size: 0.85rem;
  font-weight: 600; cursor: pointer; display: flex; align-items: center;
  justify-content: center; gap: 0.4rem; transition: opacity 0.2s;
}
...
.form-select {
  ...
  border-radius: 8px; padding: 0.65rem 0.85rem; font-family: 'DM Sans', sans-serif;
  font-size: 0.82rem; color: var(--text); outline: none; cursor: pointer;
  transition: border-color 0.2s; margin-bottom: 0.9rem;
}
.form-select:focus { border-color: var(--accent); }
.form-textarea {
  width: 100%; border: 1.5px solid var(--border); border-radius: 8px;
  padding: 0.6rem 0.85rem; font-family: 'DM Sans', sans-serif; font-size: 0.8rem;
  color: var(--text); background: var(--bg); outline: none; resize: vertical;
  min-height: 80px; transition: border-color 0.2s; line-height: 1.55; margin-bottom: 0.9rem;
}
.form-textarea:focus { border-color: var(--accent); }
.btn-save {
  width: 100%; background: var(--accent); color: #fff; border: none;
  border-radius: 8px; padding: 0.8rem; font-family: 'DM Sans', sans-serif;
...
.btn-save svg { width: 15px; height: 15px; stroke: currentColor; fill: none; stroke-width: 2.5; }
.save-success {
...
.save-success svg { width: 14px; height: 14px; stroke: var(--green); fill: none; stroke-width: 2.5; }

/* loading / error */
.state-msg { text-align: center; padding: 4rem 2rem; color: var(--text-muted); font-size: 0.9rem; }
...
function renderDetail(o) {
  ...
  if (!o) { wrap.innerHTML = '<div class="state-msg">Order not found.</div>'; return; }
  ...
  const shippingRowHtml = shippingFee > 0
    ? `<div class="total-row"><span class="total-label">Shipping (J&T Express)</span><span style="color:var(--text-muted)">₱${shippingFee.toLocaleString()}</span></div>`
    : '';
  ...
  if (from === 'payments') {
    ...
    const rejectionHtml = (o.payment?.status === 'Rejected' && o.rejectionReason)
      ? `<div class="rejection-banner" style="display:block;" id="rejection-banner">✕ Reason: ${esc(o.rejectionReason)}</div>`
      : `<div class="rejection-banner" id="rejection-banner"></div>`;
    ...
    const statusOptions = ['Processing','Shipping','Shipped','Completed'].map(s =>
  ...
  const orderInfoHtml = from === 'orders' ? `
...
async function doApprove() {
  ...
  try {
    ...
    if (data.error) { showToast(data.error); resetPaymentBtns(o); return; }
...
function doSave() {
  ...
  openConfirm(
    `Update to "${newStatus}"?`,
    `This will change the order status to "${newStatus}". The customer will see the updated status on their order page.`,
    _doSaveConfirmed,
    'Update Status',
    false
  );
...
async function _doSaveConfirmed() {
  ...
  try {
    ...
    if (data.error) { showToast(data.error); resetSaveBtn(); return; }
