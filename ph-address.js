...
(function () {
  ...
  function setOptions(selectEl, items, placeholder) {
    selectEl.innerHTML = `<option value="">${placeholder}</option>` +
      items
        ...
        .map(i => `<option value="${i.code}" data-name="${i.name}">${i.name}</option>`)
        .join('');
  ...
  const phAddr = {
    ...
    async loadProvinces() {
      ...
      try {
        if (NO_PROVINCE_REGIONS.has(regionCode)) {
          ...
          provinceEl.innerHTML = '<option value="__none__" selected>N/A — No Province</option>';
        ...
        provinceEl.innerHTML = '<option value="">Error loading provinces</option>';
    ...
    async _loadCitiesForRegion(regionCode) {
      ...
      try {
        const cities = await apiFetch(`/regions/${regionCode}/cities-municipalities/`);
    ...
    async loadCities() {
      ...
      try {
        const cities = await apiFetch(`/provinces/${provinceCode}/cities-municipalities/`);
        setOptions(cityEl, cities, 'Select City / Municipality');
      } catch (e) {
        cityEl.innerHTML = '<option value="">Error loading cities</option>';
    ...
    async loadBarangays() {
      ...
      try {
        const barangays = await apiFetch(`/cities-municipalities/${cityCode}/barangays/`);
    ...
    getFullAddress() {
      ...
      const bgy    = bgyOpt?.options[bgyOpt.selectedIndex]?.dataset.name  || bgyOpt?.value;
      const city   = cityOpt?.options[cityOpt.selectedIndex]?.dataset.name || cityOpt?.value;
      const prov   = provOpt?.options[provOpt.selectedIndex]?.dataset.name || '';
      const reg    = regOpt?.options[regOpt.selectedIndex]?.dataset.name   || regOpt?.value;
    ...
    validate() {
      if (!sel('pf-region')?.value)   { showToast('Please select a Region.');            return null; }
      if (!sel('pf-city')?.value)     { showToast('Please select a City/Municipality.'); return null; }
      if (!sel('pf-barangay')?.value) { showToast('Please select a Barangay.');          return null; }
      if (!sel('pf-street')?.value.trim()) { showToast('Please enter your street/house details.'); return null; }
    ...
    reset() {
      ['pf-region','pf-province','pf-city','pf-barangay'].forEach(id => {
        const el = sel(id);
        if (el) { el.selectedIndex = 0; el.disabled = true; }
      });
      const regionEl = sel('pf-region');
      if (regionEl) regionEl.disabled = false;
      const streetEl = sel('pf-street'); if (streetEl) streetEl.value = '';
      const zipEl    = sel('pf-zip');    if (zipEl)    zipEl.value    = '';
    },
  };
