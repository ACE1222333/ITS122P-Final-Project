...
<html lang="en">
  <head>
    ...
    <title>Reviews — BuyTheBella Admin</title>
    ...
    <style>
      ...
      .rev-product-chip-name {
        font-size: 0.82rem;
        font-weight: 500;
        line-height: 1.3;
      }
      .rev-product-chip-price {
        font-size: 0.76rem;
        color: var(--text-muted);
        margin-top: 0.15rem;
      }
  ...
  <body>
    <div class="shell">
      <main class="main">
        ...
        <div class="rev-list" id="rev-list">
          <div class="rev-empty">Loading reviews…</div>
        </div>
      </main>
    </div>
    ...
    <script>
      ...
      async function loadReviews() {
        try {
          ...
          document.getElementById("rev-list").innerHTML = `<div class="rev-empty">Failed to load reviews: ${e.message}</div>`;
        }
      }
      ...
      function applyFilters() {
        ...
        document.getElementById("rev-count-bar").textContent = `Showing ${filtered.length} of ${_allReviews.length} review${_allReviews.length !== 1 ? "s" : ""}`;
      ...
      function renderReviewList(reviews) {
        ...
        if (!reviews.length) {
          list.innerHTML = `<div class="rev-empty">No reviews match your filters.</div>`;
          return;
        }
    ...
    <div class="rev-card" id="rev-card-${r.review_id}">
      <div class="rev-card-top">
        ...
        <div class="rev-meta">
          ...
          <div class="rev-meta-sub">
            ...
            <span class="${ratingPillCls}">${r.rating}/5</span>
      ...
      async function saveReply(reviewId) {
        ...
        if (!text) {
          showAdminToast("Reply cannot be empty.");
          return;
        }
      ...
      async function clearReply(reviewId) {
        ...
        try {
          ...
          const idx = _allReviews.findIndex((r) => r.review_id === reviewId);
          if (idx !== -1) {
            _allReviews[idx].admin_reply = null;
            _allReviews[idx].reply_date = null;
          }
          _updateStats();
          applyFilters();
